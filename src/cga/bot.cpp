...
double Bot::evaluate_matchup(const SimState& base,
    ...
    for (int t = 0; t < steps; t++) {
        if (sim.game_over) {
            if (cumulative_eval) {
                double final_eval = sim.eval(eval_player) + sim.energy_proximity(eval_player) - sim.energy_proximity(1 - eval_player) + sim.height_advantage(eval_player) - sim.height_advantage(1 - eval_player) + sim.territory(eval_player) + center_control_factor * (sim.center_control(eval_player) - sim.center_control(1 - eval_player));
        ...
        if (cumulative_eval) {
            ...
            score += (sim.eval(eval_player) + sim.energy_proximity(eval_player) - sim.energy_proximity(1 - eval_player) + sim.height_advantage(eval_player) - sim.height_advantage(1 - eval_player) + sim.territory(eval_player) + center_control_factor * (sim.center_control(eval_player) - sim.center_control(1 - eval_player))) * weight;
        }
    }

    if (!cumulative_eval) {
        score = sim.eval(eval_player) + sim.energy_proximity(eval_player) - sim.energy_proximity(1 - eval_player) + sim.height_advantage(eval_player) - sim.height_advantage(1 - eval_player) + sim.territory(eval_player) + center_control_factor * (sim.center_control(eval_player) - sim.center_control(1 - eval_player));
    }
    return score;
}
